...
import { Users, TrendingUp, CheckSquare, CreditCard, AlertTriangle, ArrowRight } from 'lucide-react';
...
export default function AdminOverview() {
  ...
  const recentActivity = [
    { time:'2 mins ago',  action:'KYC Submitted',     detail:'Sunshine Ventures Ltd',  type:'kyc' },
    { time:'18 mins ago', action:'New Investment',     detail:'John Doe — ₦50M Genesis', type:'invest' },
    { time:'1 hr ago',    action:'Wallet Funded',      detail:'Prodigy Holdings — ₦1.25M', type:'wallet' },
    { time:'3 hrs ago',   action:'Loan Application',   detail:'Sarah Alabi — ₦1M',      type:'loan' },
    { time:'5 hrs ago',   action:'Redemption Request', detail:'Corp Fund — ₦2M Pre-term', type:'redeem' },
  ];

  const typeColors = { kyc:'#8b5cf6', invest:'#22c55e', wallet:'#3b82f6', loan:'#f97316', redeem:'#ef4444' };
  ...
  return (
    <div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))',gap:16,marginBottom:28 }}>
        ...
        <StatCard label="Transactions Today" value={allTransactions.length} sub="₦87.5M volume" color="#e8b84b" icon={CreditCard} onClick={()=>navigate('/admin/transactions')} />
      </div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'1fr 340px',gap:24,alignItems:'start' }}>
            ...
            {pending.slice(0,4).map(a => (
              ...
              >
                ...
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontSize:13,fontWeight:600,color:'var(--navy)',marginBottom:2 }}>{a.client}</div>
                  <div style={{ fontSize:11,color:'var(--gray-400)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{a.detail}</div>
                </div>
                <div style={{ textAlign:'right',flexShrink:0 }}>
                  ...
                  <div style={{ fontSize:10,color:'var(--gray-400)',marginTop:3 }}>{a.date}</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="animate-in delay-3" style={{ background:'white',borderRadius:14,padding:0,border:'1px solid var(--gray-200)',overflow:'hidden' }}>
          <div style={{ padding:'18px 22px',borderBottom:'1px solid var(--gray-100)' }}>
            <h3 style={{ fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:13,color:'var(--navy)',letterSpacing:'0.06em',textTransform:'uppercase' }}>Recent Activity</h3>
          </div>
          <div>
            {recentActivity.map((a,i) => (
              <div key={i} style={{ padding:'12px 20px',borderBottom:'1px solid var(--gray-100)',display:'flex',alignItems:'center',gap:12 }}>
                <div style={{ width:8,height:8,borderRadius:'50%',background:typeColors[a.type],flexShrink:0 }} />
                <div style={{ flex:1,minWidth:0 }}>
                  <div style={{ fontSize:12,fontWeight:600,color:'var(--navy)' }}>{a.action}</div>
                  <div style={{ fontSize:11,color:'var(--gray-400)',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{a.detail}</div>
                </div>
                <div style={{ fontSize:10,color:'var(--gray-400)',flexShrink:0 }}>{a.time}</div>
              </div>
            ))}
          </div>
        </div>
