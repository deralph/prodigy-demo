    ...
    <div>
      <div style={{ marginBottom: 24, display: 'flex', alignItems: 'flex-end', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }} className="animate-in">
        <div>
          ...
          <p style={{ fontSize: 11, color: 'var(--gray-400)', letterSpacing: '0.1em', textTransform: 'uppercase', marginTop: 4 }}>KYC Document Management</p>
        </div>
        <button onClick={loadKyc} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '8px 14px', background: 'rgba(13,27,53,0.06)', border: '1px solid var(--gray-200)', borderRadius: 8, cursor: 'pointer', fontSize: 12, fontWeight: 700, color: 'var(--navy)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.65)' }}>
              {uploaded}/{docs.length} documents submitted · Select each document below to upload
            </div>
          </div>
          <div>
            <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Completion</div>
            <div style={{ fontFamily: 'Syne,sans-serif', fontWeight: 800, fontSize: 22, color: 'var(--gold)' }}>{docs.length ? Math.round((uploaded / docs.length) * 100) : 0}%</div>
          </div>
        </div>
        <div style={{ marginTop: 14, height: 6, background: 'rgba(255,255,255,0.15)', borderRadius: 3, overflow: 'hidden' }}>
          <div style={{ height: '100%', width: `${docs.length ? (uploaded / docs.length) * 100 : 0}%`, background: 'var(--gold)', borderRadius: 3, transition: 'width 0.5s ease' }} />
        </div>
      </div>
          ...
          return (
            <div key={doc.key} style={{ padding: '16px 22px', borderBottom: i < docs.length - 1 ? '1px solid var(--gray-100)' : 'none', display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap', transition: 'background 0.15s' }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexShrink: 0 }}>
                ...
                {status !== 'VERIFIED' && (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '7px 12px', background: status === 'UPLOADED' ? 'rgba(59,130,246,0.1)' : status === 'REJECTED' ? 'rgba(239,68,68,0.1)' : 'var(--navy)', color: status === 'UPLOADED' ? '#3b82f6' : status === 'REJECTED' ? 'var(--red)' : 'white', borderRadius: 7, cursor: isUpl ? 'not-allowed' : 'pointer', fontSize: 11, fontWeight: 700 }}>
                    <input ref={el => fileRefs.current[doc.key] = el} type="file" accept=".pdf,.jpg,.jpeg,.png" style={{ display: 'none' }} disabled={isUpl} onChange={e => handleFileChange(doc.key, e.target.files?.[0] || null)} />
                    <Upload size={11} /> {isUpl ? 'Uploading…' : status === 'UPLOADED' ? 'Replace' : status === 'REJECTED' ? 'Re-upload' : 'Upload'}
                  </label>
                )}
                {canView && (
