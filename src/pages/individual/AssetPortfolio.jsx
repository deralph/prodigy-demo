...
import {
  ...
  Filter, BarChart2, Clock, RefreshCcw, ChevronDown, Layers, DollarSign
} from 'lucide-react';
import {
...
/* Generate per-investment monthly growth (12 months) */
...
function InvestmentDrawer({ inv, plans, user, onClose }) {
  ...
  const [termStep, setTermStep] = useState(null); // null | 'confirm' | 'done'
  const [termReason, setTermReason] = useState('');
  const [tab, setTab] = useState('overview'); // overview | chart | history | terminate
  ...
  const downloadCertificate = (terminated = false) => {
    ...
    const lines = [
      ...
      '',
      '── RETURNS ──────────────────────────────────────────',
      `Gross Return    : ${fmt2(grossReturn)}`,
      `Tax Deducted    : ${fmt2(tax)}`,
      `Net Return      : ${fmt2(netReturn)}`,
      '',
  ...
  const exportStatement = () => {
    const date = new Date().toLocaleDateString('en-GB',{day:'2-digit',month:'long',year:'numeric'});
    const rows  = (inv.history||[]).map(h=>`"${h.date}","${h.action}","${fmt2(inv.amount)}","${inv.status}"`).join('\n');
      ...
      <div style={{ width:'min(520px,100vw)', background:'white', display:'flex', flexDirection:'column', overflowY:'auto', boxShadow:'-24px 0 60px rgba(0,0,0,0.18)', animation:'slideIn 0.28s ease' }}
        ...
        <div style={{ background:color, padding:'22px 24px', flexShrink:0 }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:12 }}>
            {[
              { label:'Principal',   val:fmt2(inv.amount) },
              { label:'Net Return',  val:fmt2(netReturn) },
          ...
          {tab === 'history' && (
            <div>
              ...
              {(inv.history||[]).length === 0 && (
                <div style={{ padding:'32px', textAlign:'center', color:'var(--gray-400)', fontSize:12 }}>No transaction history available.</div>
              )}
              {(inv.history||[]).map((h,i)=>(
            ...
            <div>
              {termStep === 'done' ? (
                <div style={{ textAlign:'center', padding:'32px 0' }}>
                  ...
                  <p style={{ fontSize:12, color:'var(--gray-400)', marginBottom:20, lineHeight:1.6 }}>Your request has been submitted to our compliance team. You will be notified within 1–2 business days.</p>
                ...
                <div>
                  <div style={{ background:'rgba(239,68,68,0.07)', border:'1px solid rgba(239,68,68,0.2)', borderRadius:12, padding:'16px', marginBottom:18 }}>
                    ...
                    <p style={{ fontSize:12, color:'var(--navy)', lineHeight:1.7 }}>
                      You are requesting early termination of <strong>{inv.plan}</strong> with a principal of <strong>{fmt2(inv.amount)}</strong>.
                      Early termination may incur a penalty of up to <strong>25% of net returns</strong> per product terms.
                    </p>
                  </div>
                  ...
                  <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
                    <button onClick={()=>setTermStep(null)} style={{ padding:'13px', background:'var(--gray-100)', color:'var(--navy)', border:'none', borderRadius:10, cursor:'pointer', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:12 }}>
                      Cancel
                    </button>
                    <button onClick={()=>setTermStep('done')} style={{ padding:'13px', background:'var(--red)', color:'white', border:'none', borderRadius:10, cursor:'pointer', fontFamily:'Syne,sans-serif', fontWeight:700, fontSize:12 }}>
                      Submit Request
                    </button>
                  </div>
                </div>
                ...
                <div>
                  <div style={{ background:'rgba(239,68,68,0.06)', border:'1px solid rgba(239,68,68,0.15)', borderRadius:12, padding:'16px', marginBottom:20 }}>
                    ...
                    <ul style={{ fontSize:12, color:'var(--navy)', lineHeight:1.9, paddingLeft:16, margin:0 }}>
                      ...
                      <li>Requests must be approved by the compliance team within 1–2 business days.</li>
                      <li>Principal is returned within <strong>5 business days</strong> of approval.</li>
                      <li>A <strong>Termination Certificate</strong> will be issued upon processing.</li>
                    </ul>
                  </div>
                  <div style={{ background:'white', border:'1px solid var(--gray-200)', borderRadius:10, padding:'14px', marginBottom:18 }}>
                    {[
                      ...
                      ['Principal',       fmt2(inv.amount)],
                      ['Net Return',      fmt2(netReturn)],
                      ['Penalty (est.)',  fmt2(netReturn * 0.25)],
                      ['Net After Exit',  fmt2(inv.amount + netReturn * 0.75)],
                    ].map(([l,v])=>(
...
export default function AssetPortfolio() {
  ...
  const [chartFilter, setChartFilter] = useState('all'); // all | planId
  const [chartType, setChartType]     = useState('area'); // area | line | bar
  const [statusFilter, setStatusFilter] = useState('all');

  // For regular users clientInvestments comes from getMyInvestments() (already scoped).
  // For admins viewing a client it's filtered by clientId.
  const myInvs = user?.role === 'admin'
    ? clientInvestments.filter(i => i.clientId === user?.clientId)
    : clientInvestments;
  ...
  return (
    <div>
      <div style={{ marginBottom:24 }} className="animate-in">
        ...
        <p style={{ fontSize:11,color:'var(--gray-400)',letterSpacing:'0.1em',textTransform:'uppercase',marginTop:4 }}>Personal Investment Dashboard · {myInvs.length} Position{myInvs.length!==1?'s':''}</p>
      </div>
      ...
      <div style={{ background:'var(--navy)',borderRadius:16,padding:'26px 30px',marginBottom:22,position:'relative',overflow:'hidden' }} className="animate-in delay-1">
        ...
        <div style={{ position:'absolute',bottom:-30,left:-30,width:150,height:150,borderRadius:'50%',background:'rgba(232,184,75,0.03)' }}/>
        <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:16 }}>
          ...
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr 1fr',gap:16,textAlign:'right' }}>
            {[
              { label:'Active AUM',  val:fmt2(activeAUM) },
              { label:'Est. Net',    val:fmt2(totalNet) },
              { label:'Products',    val:uniqueProducts.length },
            ].map(s=>(
              <div key={s.label}>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(140px,1fr))',gap:14,marginBottom:22 }} className="animate-in delay-2">
        {[
          { label:'Total Invested', val:fmt2(totalAUM),    color:'var(--navy)' },
          { label:'Active',          val:activeCount,       color:'var(--green)' },
          { label:'Avg ROI',         val:`${avgRoi}%`,       color:'var(--gold)' },
          { label:'Net Returns',     val:fmt2(totalNet),   color:'#22c55e' },
          { label:'Products',        val:uniqueProducts.length, color:'#8b5cf6' },
        ].map(s=>(
      ...
      {myInvs.length > 0 && (
        <div style={{ background:'white',borderRadius:14,border:'1px solid var(--gray-200)',padding:'22px',marginBottom:22 }} className="animate-in delay-2">
          <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:10 }}>
            <div>
              <h3 style={{ fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,color:'var(--navy)',letterSpacing:'0.08em',textTransform:'uppercase' }}>Portfolio Growth by Product</h3>
              <p style={{ fontSize:11,color:'var(--gray-400)',marginTop:2 }}>Monthly capital accumulation per investment product</p>
            </div>
            <div style={{ display:'flex',gap:8,flexWrap:'wrap' }}>
              ...
              {['area','line','bar'].map(t=>(
                <button key={t} onClick={()=>setChartType(t)} style={{ padding:'6px 11px',borderRadius:7,border:`1.5px solid ${chartType===t?'var(--navy)':'#e2e8f0'}`,background:chartType===t?'var(--navy)':'white',color:chartType===t?'white':'var(--navy)',fontSize:10,fontWeight:700,cursor:'pointer',textTransform:'capitalize' }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <ResponsiveContainer width="100%" height={250}>
            {chartType === 'bar' ? (
              ...
              <LineChart data={perProductData}>
                ...
                <Legend/>
                {displayedProducts.map(p=>(
                  <Line key={p.id} type="monotone" dataKey={p.name} stroke={p.color} strokeWidth={2.5} dot={{ r:3 }}/>
                ))}
              </LineChart>
            ) : (
              <AreaChart data={perProductData}>
                ...
                <Legend/>
                {displayedProducts.map(p=>(
                  <Area key={p.id} type="monotone" dataKey={p.name} stroke={p.color} fill={p.color+'22'} strokeWidth={2}/>
                ))}
              </AreaChart>
            )}
          </ResponsiveContainer>
      ...
      {myInvs.length > 0 && (
        <div style={{ background:'white',borderRadius:14,border:'1px solid var(--gray-200)',padding:'22px',marginBottom:22 }} className="animate-in delay-3">
          ...
          <p style={{ fontSize:11,color:'var(--gray-400)',marginBottom:14 }}>Cumulative ROI growth over 12 months per product</p>
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={perProductData}>
              ...
              {uniqueProducts.map(p=>(
                <Line key={p.id} type="monotone" dataKey={p.name+'_roi'} name={p.name+' ROI'} stroke={p.color} strokeWidth={2} dot={false}/>
              ))}
            </LineChart>
          </ResponsiveContainer>
        </div>
              ...
              <div style={{ padding:'18px 22px',display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap' }}>
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(100px,1fr))',gap:'7px 18px',marginTop:6 }}>
                    {[
                      ['Principal',fmt2(inv.amount)],['ROI',`${inv.roi}%`],['Tax',`${inv.tax||0}%`],
                      ['Tenor',inv.tenor],['Value Date',inv.valueDate],['Maturity',inv.maturityDate],
                    ].map(([l,v])=>(
                      <div key={l}>
