...
import { TrendingUp, Users, Eye, X, Download } from 'lucide-react';
...
export default function JointPortfolio() {
  ...
  const exportStatement = (inv) => {
    const rows = (inv.history||[]).map(h=>`"${h.date}","${h.action}","${fmt(inv.amount)}"`);
    const blob = new Blob([`Date,Action,Amount\n${rows.join('\n')}`],{type:'text/csv'});
    const url  = URL.createObjectURL(blob);
    const a    = document.createElement('a'); a.href=url; a.download=`${inv.plan.replace(/\s/g,'_')}_joint_statement.csv`; a.click();
  ...
  return (
    <div>
      <div style={{ marginBottom:24 }} className="animate-in">
        ...
        <p style={{ fontSize:11,color:'var(--gray-400)',letterSpacing:'0.1em',textTransform:'uppercase',marginTop:4 }}>Joint Investment Dashboard · Dual-Holder Account</p>
      </div>
      ...
      <div style={{ background:'rgba(232,184,75,0.08)',border:'1px solid rgba(232,184,75,0.25)',borderRadius:10,padding:'12px 18px',marginBottom:18,display:'flex',alignItems:'center',gap:10 }} className="animate-in">
        ...
        <span style={{ fontSize:12,color:'var(--navy)',fontWeight:600 }}>
          Primary Holder: <strong>{user?.name}</strong>
          {client?.secondaryName && <> &nbsp;&bull;&nbsp; Secondary Holder: <strong>{client.secondaryName}</strong></>}
        </span>
        <span style={{ marginLeft:'auto',fontSize:10,fontWeight:700,letterSpacing:'0.06em',textTransform:'uppercase',color:'var(--gold)',background:'rgba(232,184,75,0.12)',padding:'3px 9px',borderRadius:4 }}>AND Mandate</span>
      </div>
        ...
        <div style={{ display:'flex',alignItems:'flex-start',justifyContent:'space-between',flexWrap:'wrap',gap:16 }}>
          <div>
            ...
            <p style={{ fontSize:12,color:'rgba(255,255,255,0.5)',display:'flex',alignItems:'center',gap:5 }}>
              ...
              <span style={{ color:'var(--green)',fontWeight:600 }}>{avgRoi}% Average ROI · {myInvs.length} Position{myInvs.length!==1?'s':''}</span>
            </p>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:12,textAlign:'right' }}>
            {[{label:'Active AUM',val:fmt(activeAUM)},{label:'Positions',val:`${myInvs.filter(i=>i.status==='active').length} Active`}].map(s=>(
              <div key={s.label}>
      ...
      <div style={{ display:'flex',flexDirection:'column',gap:14 }} className="animate-in delay-3">
        <h3 style={{ fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:13,color:'var(--navy)',letterSpacing:'0.06em',textTransform:'uppercase',marginBottom:4 }}>Shared Holdings</h3>
        {myInvs.length===0 && (
          <div style={{ background:'white',borderRadius:12,border:'1px solid var(--gray-200)',padding:'40px',textAlign:'center',color:'var(--gray-400)',fontSize:13 }}>No joint investments found.</div>
        )}
        {myInvs.map(inv => {
              ...
              <div style={{ padding:'18px 22px',display:'flex',alignItems:'flex-start',gap:16,flexWrap:'wrap' }}>
                <div style={{ flex:1,minWidth:0 }}>
                  ...
                  <div style={{ display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(110px,1fr))',gap:'8px 20px',marginTop:8 }}>
                    {[['Principal',fmt(inv.amount)],['ROI',`${inv.roi}%`],['Tax',`${inv.tax}%`],['Tenor',inv.tenor],['Value Date',inv.valueDate],['Maturity',inv.maturityDate]].map(([l,v])=>(
                      <div key={l}>
                ...
                <div style={{ textAlign:'right',flexShrink:0 }}>
                  ...
                  <div style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:18,color:'var(--green)',marginBottom:2 }}>{fmt(net)}</div>
                  <div style={{ fontSize:11,color:'var(--gray-400)',marginBottom:12 }}>Gross: {fmt(gross)} · Tax: {fmt(tax)}</div>
        ...
        <div style={{ position:'fixed',inset:0,background:'rgba(13,27,53,0.55)',backdropFilter:'blur(4px)',display:'flex',alignItems:'center',justifyContent:'center',zIndex:300,padding:20 }} onClick={()=>setViewing(null)}>
          <div style={{ background:'white',borderRadius:20,width:'100%',maxWidth:500,overflow:'hidden',boxShadow:'0 32px 80px rgba(13,27,53,0.25)' }} onClick={e=>e.stopPropagation()}>
            <div style={{ background:plans.find(p=>p.id===viewing.planId)?.color||'var(--navy)',padding:'20px 24px',display:'flex',alignItems:'center',justifyContent:'space-between' }}>
              <div style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:15,color:'white',textTransform:'uppercase' }}>{viewing.plan} — Joint</div>
            ...
            <div style={{ padding:'22px 24px',maxHeight:'70vh',overflowY:'auto' }}>
              {[['Investment ID',viewing.id],['Principal',fmt(viewing.amount)],['ROI Rate',`${viewing.roi}%`],['Tax',`${viewing.tax}%`],['Tenor',viewing.tenor],['Value Date',viewing.valueDate],['Maturity',viewing.maturityDate],['Status',viewing.status],['Net Return',fmt((viewing.amount*viewing.roi)/100-((viewing.amount*viewing.roi)/100*viewing.tax)/100)],['Primary Holder',user?.name||'—'],['Secondary Holder',client?.secondaryName||'—'],['Mandate Type','AND (Both Signatories)']].map(([l,v])=>(
              ...
              <button onClick={()=>exportStatement(viewing)} style={{ marginTop:16,width:'100%',padding:'12px',background:'var(--navy)',color:'white',border:'none',borderRadius:8,cursor:'pointer',fontFamily:'Syne,sans-serif',fontWeight:700,fontSize:12,display:'flex',alignItems:'center',justifyContent:'center',gap:6 }}>
                <Download size={14}/> DOWNLOAD JOINT STATEMENT
              </button>
            </div>
          </div>
        </div>
