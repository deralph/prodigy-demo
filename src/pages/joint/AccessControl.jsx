...
import { Shield, Upload, CheckCircle, Clock, FileText, Lock, Gavel, Users } from 'lucide-react';
...
const initDocState = () => Object.fromEntries(HOLDER_DOCS.map(d => [d.key, 'pending']));
...
/* ── Authority panel ── */
function AuthorityPanel({ holders, docStates, getUploaded, getPct, mandate, n }) {
  return (
...
/* ── Doc row ── */
function DocRow({ doc, status, isUploading, activeColor, onUpload, ordinal }) {
  const isUploaded = status === 'uploaded';
  return (
    <div style={{ padding:'13px 20px',display:'flex',alignItems:'center',gap:12,flexWrap:'wrap',transition:'background 0.15s' }}
    ...
    >
      ...
      <div style={{ flex:1,minWidth:0 }}>
        ...
        <div style={{ fontSize:10,color:'var(--gray-400)' }}>Required · {ordinal} Holder KYC</div>
      </div>
      <div style={{ display:'flex',alignItems:'center',gap:8,flexShrink:0 }}>
        ...
        {!isUploaded ? (
          <button onClick={onUpload} disabled={isUploading} style={{ display:'flex',alignItems:'center',gap:5,padding:'7px 11px',background:activeColor,color:'white',border:'none',borderRadius:7,cursor:'pointer',fontSize:11,fontWeight:700,opacity:isUploading?0.6:1 }}>
...
export default function AccessControl() {
  ...
  const handleUpload = (docKey) => {
    ...
    setTimeout(() => {
      ...
      if (h && updateJointHolderKyc) {
        ...
        if (newPct === 100) updateJointHolderKyc(client?.clientId, h.email, true);
      }
    }, 1400);
  };
  ...
  return (
    <div>
      <PageHeader title="Access Control" subtitle={`${n}-Holder KYC · Mandate · Signatory Management`} />
      ...
      <AuthorityPanel holders={holders} docStates={docStates} getUploaded={getUploaded} getPct={getPct} mandate={mandate} n={n} />
      ...
      <div style={{ background:'white',borderRadius:14,border:`1px solid ${HOLDER_COLORS[activeIdx]}30`,overflow:'hidden' }} className="animate-in delay-3">
        ...
        {HOLDER_DOCS.map((doc, i) => (
          <div key={doc.key} style={{ borderBottom: i < HOLDER_DOCS.length-1 ? '1px solid var(--gray-100)' : 'none' }}>
            <DocRow
              doc={doc}
              status={activeDocMap[doc.key] || 'pending'}
              isUploading={uploading.idx === activeIdx && uploading.key === doc.key}
              activeColor={HOLDER_COLORS[activeIdx]}
              onUpload={() => handleUpload(doc.key)}
              ordinal={ORDINALS[activeIdx] || `Holder ${activeIdx+1}`}
            />
          </div>
        ))}
      </div>
