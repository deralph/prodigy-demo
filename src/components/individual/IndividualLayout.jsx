...
export default function IndividualLayout() {
  ...
  return (
    <div style={{ display:'flex', minHeight:'100vh' }}>
      {sidebarOpen && <div onClick={closeSidebar} className="mob-overlay" style={{ position:'fixed',inset:0,background:'rgba(0,0,0,0.5)',zIndex:40,display:'none' }} />}
      <aside className={`ind-sidebar ${sidebarOpen?'sidebar-open':''}`} style={{ width:220,minHeight:'100vh',background:'var(--navy)',display:'flex',flexDirection:'column',position:'fixed',left:0,top:0,bottom:0,zIndex:50,transition:'transform 0.3s ease' }}>
        <div style={{ padding:'24px 20px 18px', borderBottom:'1px solid rgba(255,255,255,0.08)' }}>
          <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between' }}>
            <div>
              <div style={{ display:'flex',alignItems:'center',gap:6 }}>
                <span style={{ fontFamily:'Syne,sans-serif',fontWeight:800,fontSize:17,color:'var(--gold)',letterSpacing:'0.1em' }}>PRODIGY</span>
              </div>
        ...
        <div style={{ padding:'14px 16px',borderTop:'1px solid rgba(255,255,255,0.08)', display:'flex',alignItems:'center',gap:10 }}>
          ...
          <div style={{ flex:1,minWidth:0 }}>
            <div style={{ fontSize:11,fontWeight:700,color:'white',fontFamily:'Syne,sans-serif',whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{user?.name||'John Doe'}</div>
            <div style={{ fontSize:9,color:'var(--gold)',letterSpacing:'0.08em',textTransform:'uppercase' }}>Single Verified</div>
          </div>
        ...
        <header style={{ height:52,background:'white',borderBottom:'1px solid var(--gray-200)',display:'flex',alignItems:'center',justifyContent:'flex-end',padding:'0 28px',gap:12,position:'sticky',top:0,zIndex:30 }}>
          ...
          <div style={{ display:'flex',alignItems:'center',gap:8,background:'var(--gray-50)',border:'1px solid var(--gray-200)',borderRadius:8,padding:'5px 10px' }}>
            ...
            <div>
              ...
              <div style={{ fontSize:11,fontWeight:700,color:'var(--navy)',fontFamily:'Syne,sans-serif' }}>{user?.clientId||'IND-2764'}</div>
            </div>
          </div>
        </header>
      ...
      <style>{`
        .ind-sidebar{transform:translateX(0);}
        .ind-content{flex:1;margin-left:220px;display:flex;flex-direction:column;min-height:100vh;}
        @media(max-width:768px){
          .ind-sidebar{transform:translateX(-100%)!important;width:220px!important;}
          .ind-sidebar.sidebar-open{transform:translateX(0)!important;}
          .sidebar-close{display:flex!important;align-items:center;}
          .mob-overlay{display:block!important;}
          .ind-content{margin-left:0!important;}
